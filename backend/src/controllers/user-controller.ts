...
export const createUser = async (
  ...
  try {
    ...
    const normalizedRole =
      typeof role === "string" ? role.toLowerCase().trim() : undefined;
    const normalizedDni =
      dni === undefined || dni === null || dni === "" ? undefined : Number(dni);
    ...
    if (normalizedRole && normalizedRole !== "client") {
      ...
      if (!token) {
        return res
          .status(403)
          .json({ message: "Admin token required to assign staff roles" });
      }
      ...
      try {
        ...
        if (!requestingUser || requestingUser.role !== "admin") {
          return res
            .status(403)
            .json({ message: "Only admin users can assign staff roles" });
        }
      } catch (error: any) {
        return res
          .status(401)
...
export const activateUser = async (
  ...
  const frontendUrl =
    ...
    process.env.FRONTEND_BASE_URL.trim().length > 0
  ...
  try {
    ...
    if (
      ...
      return res.redirect(
        `${frontendUrl}/activate/${token}?error=invalid_token`
      );
    }
    ...
    return res.redirect(
      `${frontendUrl}/activate/${req.params.token}?error=server_error`
    );
  }
};
...
export const updateForgottenPassword = async (
  ...
  try {
    ...
    const user = await User.findOne({
      where: { passwordResetTokenHash: token },
    });
